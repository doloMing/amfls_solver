...
// Non-owning dense FP64 column-major operator. The caller owns values and
// must keep every entry unchanged for the operator's lifetime: validation
// norm bounds are scanned and cached by the constructor.
class DenseOperator final : public MatrixOperator {
public:
    DenseOperator(int rows, int cols, const double* values);
